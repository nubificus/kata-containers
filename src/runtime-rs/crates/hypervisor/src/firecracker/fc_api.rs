...
impl FcInner {
    ...
    pub(crate) async fn patch_container_rootfs(
        ...
        .to_string();
        self.patch_with_retry(&["/drives/", &format!("drive{drive_id}")].concat(), body)
            .await?;
        Ok(())
    }
    ...
    pub(crate) async fn put_with_retry(&self, uri: &str, data: String) -> Result<()> {
        ...
        self.send_request_with_retry(Method::PUT, url, data).await
    }
    pub(crate) async fn patch_with_retry(&self, uri: &str, data: String) -> Result<()> {
        let url: hyper::Uri = Uri::new(&self.asock_path, uri).into();
        self.send_request_with_retry(Method::PATCH, url, data).await
    }
