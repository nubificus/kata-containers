...
impl FcInner {
    ...
    pub(crate) async fn wait_api_socket(&mut self, id: &str) -> Result<()> {
    ...
    pub(crate) async fn prepare_hvsock(&mut self, id: &str) -> Result<()> {
        ...
        self.request_with_retry(Method::PUT, "/vsock", body_vsock).await?;
        Ok(())
    }
    ...
    pub(crate) async fn prepare_vmm_resources(&mut self, id: &str) -> Result<()> {
        ...
        self.request_with_retry(Method::PUT, "/boot-source", body_kernel).await?;
        self.request_with_retry(Method::PUT, "/drives/rootfs", body_rootfs).await?;
        ...
        for i in 1..DISK_POOL_SIZE {
            let full_path_name = format!("{}/drive{}", abs_dummy_path, i);
            ...
            self.request_with_retry(Method::PUT, &format!("/drives/drive{}", i), body_dummy)
                .await?;
        }
    ...
    pub(crate) async fn patch_container_rootfs(
        ...
        .to_string();
        self.request_with_retry(Method::PATCH, &["/drives/", &format!("drive{drive_id}")].concat(), body)
            .await?;
        Ok(())
    }
    ...
    pub(crate) async fn add_net_device(
        ...
        .to_string();
        self.request_with_retry(Method::PUT, &["/network-interfaces/", &device_id].concat(), body)
            .await?;
        Ok(())
    }

    pub(crate) async fn request_with_retry(&self, method: Method,  uri: &str, data: String) -> Result<()> {
