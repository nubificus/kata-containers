...
const FC_API_SOCKET_NAME: &str = "firecracker.socket";
...
pub(crate) fn get_api_socket_path(id: &str, jailed: bool, relative: bool) -> Result<String> {
    ...
    let path = match jailed {
        ...
        true => match relative {
            ...
            .join("/"),
            true => ["firecracker", id, "root", "run", FC_API_SOCKET_NAME].join("/"),
        },
    };
    Ok(path)
}
...
pub(crate) fn get_vsock_path(id: &str, jailed: bool, relative: bool) -> Result<String> {
    ...
    let path = match jailed {
        ...
        true => match relative {
            false => [&sb_path, "firecracker", id, "root", FC_AGENT_SOCKET_NAME].join("/"),
